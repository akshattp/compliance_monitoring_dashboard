...
def get_agent_kpis(df: pd.DataFrame, agent_col: str, branch_col: str, benef_col: str) -> dict:
    ...
    total_net_amt_all = df['Net Amt'].sum(min_count=1) if 'Net Amt' in df.columns else 0
    total_net_amt_agent = agent_df['Net Amt'].sum(min_count=1) if 'Net Amt' in agent_df.columns else 0
    ...
    seg_name, seg_count, seg_pct = get_top_agent_entity(agent_df, 'Segments', agent_col, total_agents)
    prod_name, prod_count, prod_pct = get_top_agent_entity(agent_df, 'Product', agent_col, total_agents)
    purp_name, purp_count, purp_pct = get_top_agent_entity(agent_df, 'Purpose', agent_col, total_agents)
    ...
    ctry_name, ctry_count, ctry_pct = get_top_agent_entity(agent_df, 'Visiting Country', agent_col, total_agents)
...
def get_agent_frequency_table(df_in: pd.DataFrame, group_col: str, y_col: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    agg_df = df_in.groupby(group_col).agg(Count=('Net Amt', 'size'), Net_Amt=('Net Amt', 'sum')).reset_index()
    ...
    display_table = display_table.rename(columns={'Net_Amt': 'Net Amount', group_col: 'Category'})
    
    return agg_df, display_table

def get_agent_trend_table(agent_df: pd.DataFrame, trend_agg: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    if 'Date' not in agent_df.columns:
        return pd.DataFrame(), pd.DataFrame()
        
    if trend_agg == 'DAILY':
        trend_df = agent_df.groupby(agent_df['Date'].dt.date).agg(Count=('Net Amt', 'size'), Net_Amt=('Net Amt', 'sum')).reset_index()
        trend_df.rename(columns={'Date': 'Time'}, inplace=True)
    else:
        if 'Week' in agent_df.columns:
            trend_df = agent_df.groupby('Week').agg(Count=('Net Amt', 'size'), Net_Amt=('Net Amt', 'sum')).reset_index()
            trend_df.rename(columns={'Week': 'Time'}, inplace=True)
        else:
            trend_df = agent_df.groupby(pd.Grouper(key='Date', freq='W-MON')).agg(Count=('Net Amt', 'size'), Net_Amt=('Net Amt', 'sum')).reset_index()
    ...
    display_table = pd.concat([table_data, total_row], ignore_index=True)
    display_table = display_table.rename(columns={'Net_Amt': 'Net Amount', 'Time': 'Category'})
...
def get_suspicious_agents_many(df_sub: pd.DataFrame, group_col: str, target_col: str, threshold: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    grp = df_sub.groupby(group_col).agg(
        ...
        txn_count=('Net Amt', 'size'),
        total_amt=('Net Amt', 'sum')
    ).reset_index()
...
def get_suspicious_agents1_one_relation(beneficiary_df: pd.DataFrame, agent_col: str, benef_col: str, threshold: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    total_amt = suspicious_pairs['Net Amount'].sum()
