...
def get_fatf_kpis(flagged: pd.DataFrame, filtered_df: pd.DataFrame) -> dict:
    ...
    total_amount = filtered_df['Net Amt'].sum(min_count=1) if (not filtered_df.empty and 'Net Amt' in filtered_df.columns) else 0
    contrib_pct = (len(flagged) / total_count * 100) if total_count > 0 else 0
    flagged_amt = flagged['Net Amt'].sum(min_count=1) if (not flagged.empty and 'Net Amt' in flagged.columns) else 0
    ...
    return {
        ...
        'affected_branches': flagged['Branch Name'].nunique() if (not flagged.empty and 'Branch Name' in flagged.columns) else 0,
        'affected_countries': flagged['Visiting Country'].nunique() if (not flagged.empty and 'Visiting Country' in flagged.columns) else 0
    }
