...
def get_branch_exposure(enriched_df: pd.DataFrame) -> list:
    ...
    res.rename(columns={branch_col: 'Branch Name'}, inplace=True)
    return clean_for_json(res)

def get_country_exposure(enriched_df: pd.DataFrame) -> list:
    if 'Visiting Country' not in enriched_df.columns or 'Risk Classification' not in enriched_df.columns:
        return []
    res = enriched_df.groupby(['Visiting Country', 'Risk Classification']).agg(
        Transaction_Count=('Net Amt', 'size'),
        Net_Amt=('Net Amt', 'sum')
    ).reset_index().sort_values('Net_Amt', ascending=False)
    return clean_for_json(res)
...
def get_trend_exposure(enriched_df: pd.DataFrame, trend_agg: str) -> list:
    if 'Date' not in enriched_df.columns or 'Risk Classification' not in enriched_df.columns:
        return []
    ...
    if trend_agg == "DAILY":
        trend_data = df_copy.groupby([df_copy['Date'].dt.date, 'Risk Classification']).agg(
            Transaction_Count=('Net Amt', 'size'), 
            Net_Amt=('Net Amt', 'sum')
        ).reset_index()
        trend_data.rename(columns={'Date': 'Time'}, inplace=True)
    else:
        trend_data = df_copy.groupby([pd.Grouper(key='Date', freq='W-MON'), 'Risk Classification']).agg(
            Transaction_Count=('Net Amt', 'size'), 
            Net_Amt=('Net Amt', 'sum')
        ).reset_index()
