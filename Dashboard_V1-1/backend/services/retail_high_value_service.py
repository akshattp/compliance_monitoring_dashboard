...
def calculate_kpis(high_value_df: pd.DataFrame) -> dict:
    ...
    if high_value_df.empty:
        ...
        kpis['total_high_value_amount'] = 0
        kpis['highest_usd_txn'] = 0
        kpis['avg_usd_txn'] = 0
    ...
    kpis['total_high_value_amount'] = high_value_df['EQV USD'].sum()
    kpis['highest_usd_txn'] = high_value_df['EQV USD'].max()
    kpis['avg_usd_txn'] = high_value_df['EQV USD'].mean()
    
    kpis['high_risk_count'] = len(high_value_df[high_value_df['Retail_Risk_Level'] == 'HIGH'])
    kpis['medium_risk_count'] = len(high_value_df[high_value_df['Retail_Risk_Level'] == 'MEDIUM'])
    
    if 'Branch Name' in high_value_df.columns:
        top_branch = high_value_df.groupby('Branch Name')['EQV USD'].sum().idxmax()
        kpis['high_exposure_branch'] = top_branch
    else:
        kpis['high_exposure_branch'] = 'N/A'
    
    if 'Corporate' in high_value_df.columns:
        top_corp = high_value_df.groupby('Corporate')['EQV USD'].sum().idxmax()
        kpis['high_exposure_corporate'] = top_corp
    else:
        kpis['high_exposure_corporate'] = 'N/A'
    
    return kpis

def branch_wise_analysis(high_value_df: pd.DataFrame) -> pd.DataFrame:
    if 'Branch Name' not in high_value_df.columns or high_value_df.empty:
        return pd.DataFrame()
    
    return high_value_df.groupby('Branch Name').agg(
...
def corporate_wise_analysis(high_value_df: pd.DataFrame) -> pd.DataFrame:
    if 'Corporate' not in high_value_df.columns or high_value_df.empty:
        return pd.DataFrame()
    
    return high_value_df.groupby('Corporate').agg(
        Transaction_Count=('EQV USD', 'size'),
        Total_USD=('EQV USD', 'sum'),
        Avg_USD=('EQV USD', 'mean'),
        Max_USD=('EQV USD', 'max'),
...
def customer_concentration(high_value_df: pd.DataFrame) -> pd.DataFrame:
    if 'Passenger Name' not in high_value_df.columns or high_value_df.empty:
        return pd.DataFrame()
    
    return high_value_df.groupby('Passenger Name').agg(
        Transaction_Count=('EQV USD', 'size'),
        Total_USD=('EQV USD', 'sum'),
        Avg_USD=('EQV USD', 'mean'),
        Max_USD=('EQV USD', 'max'),
...
def product_wise_analysis(high_value_df: pd.DataFrame) -> pd.DataFrame:
    if 'Product' not in high_value_df.columns or high_value_df.empty:
        return pd.DataFrame()
    
    return high_value_df.groupby('Product').agg(
        Transaction_Count=('EQV USD', 'size'),
        Total_USD=('EQV USD', 'sum'),
        Avg_USD=('EQV USD', 'mean'),
    ).reset_index().sort_values('Total_USD', ascending=False)

def currency_wise_analysis(high_value_df: pd.DataFrame) -> pd.DataFrame:
    if 'Currency' not in high_value_df.columns or high_value_df.empty:
        return pd.DataFrame()
    
    return high_value_df.groupby('Currency').agg(
        Transaction_Count=('EQV USD', 'size'),
        Total_USD=('EQV USD', 'sum'),
        Avg_USD=('EQV USD', 'mean'),
    ).reset_index().sort_values('Total_USD', ascending=False)
...
def generate_observations(high_value_df: pd.DataFrame, kpis: dict) -> str:
    ...
    avg_usd = high_value_df['EQV USD'].mean()
    if avg_usd > 50000:
        observations.append(
...
def format_transaction_table(high_value_df: pd.DataFrame) -> pd.DataFrame:
    if high_value_df.empty:
        return pd.DataFrame()
