...
def get_home_kpis(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        'ps_count': ps_count,
        'pb_count': pb_count,
        'ps_count_pct': ps_count_pct,
        'ps_amt_pct': ps_amt_pct,
        'pb_count_pct': pb_count_pct,
        'pb_amt_pct': pb_amt_pct,
        ...
        'best_segment_amt': best_segment_amt,
        'best_segment_pct': best_segment_pct,
        'best_branch_name': best_branch_name,
        'best_branch_amt': best_branch_amt,
        'best_branch_pct': best_branch_pct,
    }
...
def get_home_trends(df: pd.DataFrame, trend_agg: str) -> dict:
    ...
    highest_amount_time = time_amount.iloc[0] if not time_amount.empty else None
    lowest_amount_time = time_amount.iloc[-1] if not time_amount.empty else None
    highest_count_time = time_count.iloc[0] if not time_count.empty else None
    lowest_count_time = time_count.iloc[-1] if not time_count.empty else None
...
def get_home_breakdowns(df: pd.DataFrame, is_count: bool, purpose_threshold: float) -> dict:
    ...
    return {
        ...
        'purpose_summary_table': purpose_summary_table,
        'product_df': product_df,
        'product_summary_table': product_summary_table,
        'branch_df': branch_df,
